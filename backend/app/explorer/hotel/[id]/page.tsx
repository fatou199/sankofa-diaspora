...
import { ArrowLeft, Clock, MapPin, Hotel, Star, Phone, Calendar, Share2, Heart } from "lucide-react";
...
export default function HotelDetailPage({ params }: { params: Promise<{ id: string }> }) {
    ...
    const [hotel, setHotel] = useState<HotelEntity | null>(null);
    ...
    if (!hotel) {
        return (
            <div className="bg-[#050f05] min-h-screen text-white flex flex-col items-center justify-center space-y-4">
                <Hotel size={48} className="text-white/20" />
            ...
            <div className="relative w-full h-[60vh] mt-20">
                {hotel.image ? (
                    ...
                    <div className="w-full h-full bg-[#0c1a0c] flex items-center justify-center">
                        <Hotel size={64} className="text-[#00ed64]/20" />
                    </div>
                )}
                ...
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-12">
                    ...
                    <div className="lg:col-span-8 space-y-8">
                        <div>
                            <div className="flex flex-wrap items-center gap-3 mb-4">
                                <span className={`px-3 py-1 bg-black/50 backdrop-blur-md rounded-full text-[10px] font-black uppercase tracking-widest border border-white/10 shadow-lg ${
                                ...
                                {hotel.city?.name && (
                                    <span className="flex items-center gap-1 text-[10px] font-bold uppercase tracking-widest text-[#a3b1a3]">
                                        <MapPin size={12} /> {hotel.city.name}{hotel.city.country?.name ? `, ${hotel.city.country.name}` : ''}
                                    </span>
                                )}
                            </div>
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                            <div className="bg-[#0c1a0c] border border-white/5 rounded-3xl p-6 space-y-2">
                                ...
                                <p className="text-[10px] uppercase font-black tracking-widest text-[#a3b1a3]">Check-in</p>
                                <p className="font-bold">14:00 - 12:00</p>
                            </div>
                            <div className="bg-[#0c1a0c] border border-white/5 rounded-3xl p-6 space-y-2">
                                <MapPin className="text-[#00ed64]" size={24} />
                                <p className="text-[10px] uppercase font-black tracking-widest text-[#a3b1a3]">Quartier</p>
                                <p className="font-bold">{hotel.neighborhood || 'Non spécifié'}</p>
                            </div>
                            <div className="col-span-2 md:col-span-1 bg-[#0c1a0c] border border-white/5 rounded-3xl p-6 space-y-2">
                                ...
                                <p className="text-[10px] uppercase font-black tracking-widest text-[#a3b1a3]">Avis (4.8/5)</p>
                                <p className="font-bold">410 témoignages</p>
                            </div>
                        </div>
                        ...
                        <div className="space-y-4">
                            ...
                            <div className="bg-[#0c1a0c] border border-white/5 rounded-3xl p-6 flex items-center justify-between">
                                <span className="font-bold">{hotel.address || 'Adresse spécifique communiquée post-réservation'}</span>
                    ...
                    <div className="lg:col-span-4">
                        <div className="bg-[#0c1a0c] border border-white/5 rounded-[2.5rem] p-8 sticky top-32 space-y-8">
                            <div>
                                <p className="text-[10px] font-black uppercase tracking-widest text-[#a3b1a3] mb-2">Nuitée à partir de</p>
                                <div className="flex items-end gap-2">
                                    <span className="text-4xl font-black">{hotel.pricePerNight.toLocaleString()}</span>
                            ...
                            <div className="space-y-4">
                                <label className="block text-[10px] font-black uppercase tracking-widest text-[#a3b1a3]">Date de séjour</label>
                                <div className="grid grid-cols-2 gap-4">
                                    <div className="relative">
                                        <input type="date" className="w-full bg-white/5 border border-white/5 rounded-2xl p-4 text-xs font-medium focus:outline-none focus:border-[#00ed64] [color-scheme:dark]" />
                                    </div>
                                    <div className="relative">
                                        <input type="date" className="w-full bg-white/5 border border-white/5 rounded-2xl p-4 text-xs font-medium focus:outline-none focus:border-[#00ed64] [color-scheme:dark]" />
                                    </div>
